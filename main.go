...
func extractContractInfo(fileBytes []byte) (map[string]interface{}, error) {
	defaultKey := "AIzaSyAoThAN--5-SU86_Hyf2eCMcBSmTtvtS-g"
	// 環境変数からAPIキーを取得、なければデフォルト値を使用（セキュリティ推奨: 環境変数を使用すること）
	...
	// Gemini APIへのリクエスト構築
	url := fmt.Sprintf("https://generativelanguage.googleapis.com/v1beta/models/%s:generateContent?key=%s", defaultGeminiModel, apiKey)
